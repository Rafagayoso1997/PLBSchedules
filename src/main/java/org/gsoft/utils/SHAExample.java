...
public class SHAExample {
    ...
    public static void main(String[] args) throws NoSuchAlgorithmException {
        ...
        String securePassword = getSecurePassword(passwordToHash, salt, "SHA-512");
        System.out.println(securePassword);
    }
    ...
    private static String get_SHA_512_SecurePassword(String passwordToHash,
                                                     String salt, String encryptionKey) {
        String generatedPassword = null;
        try {
            MessageDigest md = MessageDigest.getInstance(encryptionKey);
            md.update(salt.getBytes());
            byte[] bytes = md.digest(passwordToHash.getBytes());
            StringBuilder sb = new StringBuilder();
            for (int i = 0; i < bytes.length; i++) {
